...
data "aws_s3_bucket_object" "count_lambda_zip" {
 bucket = var.artifact_bucket_name
  key    = "${terraform.workspace}/count_notifier_lambda.zip"
}


resource "aws_lambda_function" "count_notifier_lambda" {
  function_name = "predict_etl_${terraform.workspace}_count_notifier_lambda"
  ...
  s3_bucket         = data.aws_s3_bucket_object.count_lambda_zip.bucket
  s3_key            = data.aws_s3_bucket_object.count_lambda_zip.key
  s3_object_version = data.aws_s3_bucket_object.count_lambda_zip.version_id
 vpc_config {
