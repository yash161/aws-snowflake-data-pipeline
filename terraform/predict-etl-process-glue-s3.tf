resource "aws_glue_job" "s3_source_glue_job" {
  name = "predict-etl-${terraform.workspace}-s3-to-snowflake-sync"
  ...
  command {
    name = "gluestreaming"
    script_location = "s3://${var.artifact_bucket_name}/${terraform.workspace}/src/glue/s3_to_snowflake_sync/main.py"
    python_version  = 3
  }
  execution_property {
  ...
  default_arguments = {
    ...
    "--extra-py-files" = "s3://${var.artifact_bucket_name}/${terraform.workspace}/s3_to_snowflake_sync.zip"
