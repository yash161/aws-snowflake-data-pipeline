...
data "aws_s3_bucket_object" "source_to_snowflake_sync_lambda_zip" {
 bucket = var.artifact_bucket_name
  key    = "${terraform.workspace}/source_to_snowflake_sync.zip"
}


resource "aws_lambda_function" "lambda_postgres_to_snowflake_sync" {
  function_name = "predict_etl_${terraform.workspace}_lambda_postgres_to_snowflake_sync"
  ...
  layers = [
    aws_lambda_layer_version.postgres_to_snowflake_sync_layer.arn,
    aws_lambda_layer_version.tenacity_layer.arn
  ]
  s3_bucket         = data.aws_s3_bucket_object.source_to_snowflake_sync_lambda_zip.bucket
  s3_key            = data.aws_s3_bucket_object.source_to_snowflake_sync_lambda_zip.key
  s3_object_version = data.aws_s3_bucket_object.source_to_snowflake_sync_lambda_zip.version_id
 vpc_config {
    ...
    security_group_ids = [data.aws_security_group.predict-lmb-sg.id]
  }
}
