...
resource "aws_sfn_state_machine" "lambda_sfn_state_machine" {
  name     = "predict-etl-lambda-${terraform.workspace}-step-function"
  ...
  definition = jsonencode({
    ...
    "States" : {
      "Lambda Invoke" : {
        ...
        "Parameters" : {
          "Payload.$" : "$",
          "FunctionName" : aws_lambda_function.lambda_postgres_to_snowflake_sync.arn
        },
        "Retry" : [
          {
