...
def start_process(src_type, src_db, src_schema, src_table, primary_key, snowflake_db, snowflake_schema,
                  ...
                  aidbox_postgres_secret_name, sns_secret_name,glue_job_name):
    ...
    try:
        ...
        subject = f"Glue-Job [Env:{env}] [JobName:{glue_job_name}] [Status:{sns_dict.get('status')}] [src_db:{sns_dict.get('src_db')}] [src_schema:{sns_dict.get('src_schema')}] [src_table:{sns_dict.get('src_table')}]"
...
if __name__ == "__main__":
    ...
    print("Starting Glue Job frm CICD")
    if check_platform() == "Linux":
        args = getResolvedOptions(sys.argv,
                                  ['src_schema', 'snowflake_db', 'snowflake_schema', 'snowflake_table', 'src_table',
                                   'src_type', 'src_db', 'primary_key', 'connect_postgres_secret_name',
                                   'aidbox_postgres_secret_name', 'snowflake_secret_name', 'sns_secret_name','glue_job_name'])
    else:
        # For testing connect postgres
        args = {'src_type': "connect_postgres",
                'src_db': "chpostgres",
                'src_schema': "public",
    ...
    connect_postgres_secret_name = args["connect_postgres_secret_name"]
    aidbox_postgres_secret_name = args["aidbox_postgres_secret_name"]
